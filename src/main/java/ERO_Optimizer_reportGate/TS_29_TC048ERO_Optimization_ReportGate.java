...
public class TS_29_TC048ERO_Optimization_ReportGate extends Keywords{
	
	public void Scenario29(WebDriver driver, ExtentTest test, ExtentTest test1, String url, int dataset) throws AWTException, IOException {
		
		System.out.println("url : " + url);
		String Username = Utils.getDataFromTestData("TS_029_TC048ERO_Optimization_ReportGate_Dataset"+dataset, "ReportGate","Username");
		String Password = Utils.getDataFromTestData("TS_029_TC048ERO_Optimization_ReportGate_Dataset"+dataset, "ReportGate","Password");
		String ID = Utils.getDataFromTestData("TS_029_TC048ERO_Optimization_ReportGate_Dataset"+dataset,
				"ReportGate", "Id");
    	String  Repo_Msg_popup = Utils.getDataFromTestData("TS_029_TC048ERO_Optimization_ReportGate_Dataset"+dataset, "ReportGate", "Forecast_Error_msg");
		...
		Extent_cal(test, "Testcase 48 - Ensure that while click the forecast plan without select a row is allow to display a validation msg select the rows to export");	
	...
	Fluentwait(driver, Forecast_Button);
	click(driver, Forecast_Button);
...
Fluentwait(driver, Error_msg_ERO_1);
	...
	if (Repopopup_text.equals(Repo_Msg_popup)) {
		...
		System.out.println(
				"Not Matched || Expected validation Message while click without selected a row was  : " + Repo_Msg_popup
						+ " Actual validation Message while click without selected a row was : " + Repopopup_text);
